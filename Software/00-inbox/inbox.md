...
- [ ] criar nota [[Composite Index]] — quando um índice `(a, b)` substitui dois índices separados- [ ] [[Teorema CAP]] — dizer que P não é opcional em sistema distribuído: a escolha real é CP ou AP, e "CA" só descreve um nó só
- [ ] [[Teorema PACELC]] — explicar o mecanismo do `E`: replicação síncrona (EC) vs assíncrona (EL), e que a sigla é da *configuração*, não do produto (Postgres com réplica async = PC/EL)
